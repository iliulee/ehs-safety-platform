...
## 二十九、UI/UX 重构方案（产品经理视角，待用户确认）

> 状态：**待确认**。确认后执行，执行完成后再将本节状态改为「已实施」并补充到变更记录。
...
**隐患治理（HazardListPage）**
- 顶部一行：标题+数量 / 搜索 / 筛选下拉 / 新增。
- 状态胶囊保留一行：全部 / 待整改(N) / 整改中(N) / 复查中(N) / 已闭环。
- 卡片增加「整改」「复查」「删除」快捷操作（根据状态显示）。
- 拆分：Page + Hook + HazardCard + HazardFilter + HazardFormSheet + HazardDetailSheet + RectifySheet + ReviewSheet。
- `useLiveQuery` 拉取隐患列表。

**人员管理（WorkerListPage）**
- 顶部一行：标题+数量 / 搜索 / 状态下拉 / 新增。
- 卡片增加「编辑」「标记离场」「删除」快捷操作。
- 拆分：Page + Hook + WorkerCard + WorkerFormSheet + WorkerDetailSheet。
- `useLiveQuery` 拉取人员和分包商。

**项目管理（ProjectListPage）**
- 顶部一行：标题+数量 / 搜索 / 新增。
- 当前项目卡片高亮，增加「切换到此项目」快捷按钮。
- 拆分：Page + Hook + ProjectCard + ProjectFormSheet + ProjectDetailSheet。
- `useLiveQuery` 拉取项目列表。

**危大工程（HazardEnginePage）**
- 顶部一行：标题+数量 / 搜索 / 状态+级别筛选 / 新增。
- 卡片增加「编辑」「删除」快捷操作。
- 拆分：Page + Hook + HazardEngineCard + HazardEngineFormSheet + HazardEngineDetailSheet。
- `useLiveQuery` 拉取列表。
