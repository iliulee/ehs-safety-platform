...
async function inferMapping(name: string): Promise<VariableMapping> {
  ...
  if (matched) {
    const base: VariableMapping = {
      name,
      label: matched.label,
      source: matched.source === 'project' ? 'field' : matched.source === 'currentDate' ? 'currentDate' : 'manual',
      required: matched.source !== 'currentDate',
    }
    if (matched.source === 'project' && matched.projectField) {
      base.field = matched.projectField
    }
    if (matched.source === 'project' && matched.extraFieldKey) {
      base.extraFieldKey = matched.extraFieldKey
    }
    return base
  }

  if (lower === 'currentdate') {
...
async function buildPreview(
  ...
  if (kind === 'unsupported') {
    throw new Error('旧版 .doc 格式无法直接处理，请在 WPS/Word 中另存为 .docx 后再导入')
  }
